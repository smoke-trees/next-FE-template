...
export async function middleware(request: NextRequest) {
	...
	if (request.method === 'GET') {
		...
		const shouldSkipAuth =
			unAuthenticatedRoutes.filter((e) => pathname.startsWith(e)).length > 0
		...
		if (
			...
			const tokenSplit = JSON.parse(
				Buffer.from(
					refreshedTokens.accessToken.split('.')[1],
					'base64'
				).toString('utf-8')
			)
			const refreshTokenSplit = JSON.parse(
				Buffer.from(
					refreshedTokens.refreshToken.split('.')[1],
					'base64'
				).toString('utf-8')
			)
			...
			response.cookies.set(
				'tokenExpiryDate',
				new Date(tokenSplit.exp * 1000).toISOString(),
				{
					maxAge: tokenSplit.exp * 1000 - Date.now(),
					httpOnly: true
				}
